...
#             torch.empty(inp.shape, dtype=torch.float32, device=inp.device)
#         ]
...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    # inp = torch.randn(1, 64, 64, 1280).cuda()
    inp = torch.zeros(1, 64, 64, 1280).float().cuda()
    ...
    model_config = ModelConfig(model_name="sam")
